package com.topscore.omnichannel.order.site.model;
...
public class DataSourceRequest {
    ...
    private static void restrict(Junction junction, FilterDescriptor filter, Class<?> clazz) {
        ...
        {            
            ...
            }catch (IntrospectionException | NumberFormatException | ParseException ignored) {
            }
        }
    ...
    private List<?>  group(final Criteria criteria, final Session session, final Class<?> clazz) {
        ...
        if (group != null && !group.isEmpty()) {
            try {
                result = groupBy(criteria.list(), group, clazz, session, new ArrayList<>());
            } catch (IllegalAccessException | InvocationTargetException | IntrospectionException | HibernateException e) {
                e.printStackTrace();
            }
        }
        return result;
